...
def get_next_batch(experience,model,num_actions,gamma,batch_size):
	...
	for i in range(len(batch)):
		...
		Q_sa = np.max(model_.predict(s_tp1)[0])
		if game_over:
			Y[i,a_t] = r_t
		else:
...
model_ = Sequential()
model_.add(Conv2D(32,kernel_size=8,strides=4,padding="same",kernel_initializer="normal",input_shape=(100,100,4)))
model_.add(Activation('relu'))
model_.add(Conv2D(64,kernel_size=4,strides=2,kernel_initializer="normal",padding="same"))
model_.add(Activation("relu"))
model_.add(Conv2D(64,kernel_size=3,strides=1,kernel_initializer="normal",padding="same"))
model_.add(Activation('relu'))
model_.add(Flatten())
model_.add(Dense(512,kernel_initializer="normal"))
model_.add(Activation('relu'))
model_.add(Dense(3,kernel_initializer="normal"))

model_.compile(optimizer=Adam(lr=1e-6),loss='mse')
...
for e in range(1,NUM_EPOCHS+1):
	...
	while not game_over:
		...
		if UPDATE_TARGET_FREQUENCY % e == 0:
			updateTargetModel()
